...
data = pd.read_csv(r"D:\NFRP\kronodroid-dataset-2008-to-2020.csv")
data.head()
...
data.drop(columns=['NrContactedIps'], inplace=True)
...
k_best = SelectKBest(score_func= chi2, k=88)
...
pca = PCA(n_components=88)  # Set the desired number of components
...
n_top_features = 2  # You can choose how many top features you want per component
top_features = set()

for i in range(pca_components.shape[0]):  # Iterate over each component
    component = pca_components[i]
    top_indices = np.argsort(np.abs(component))[-n_top_features:]  # Get indices of top features
    top_features.update(feature_names[top_indices])  # Add the top features to the set

# Convert the set to a sorted list
top_features = sorted(top_features)

print("Top contributing feature names:", top_features)
